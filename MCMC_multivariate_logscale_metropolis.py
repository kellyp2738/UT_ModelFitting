...
def BindLogs(accepted_log_names, iteration_log, model_out_log, prev_log, lik_log, prior_log, post_log, t_eq_log, accept_rate_log, is_updated_log, Paccept_reject_log, proposed_log_names, prop_iteration_log, prop_model_out_log, prop_prev_log, prop_lik_log, prop_prior_log, prop_post_log, update_index_log, par_bounds_log, prop_lik_bounds_log, prop_t_eq_log): # all the logs
    ...
    proposed_log=np.concatenate((prop_iteration_log, prop_model_out_log, prop_prev_log, prop_lik_log, prop_prior_log, prop_post_log, update_index_log, par_bounds_log, prop_lik_bounds_log, prop_t_eq_log), axis = 1)
...
def MCMC(prev_data, dir_name, burnin, iterations, pop_sizes, racc_pop, plot = False):
    ...
    # ----------------------------------------------------------------------------------------------
    # -- Step size adjustment logs
    # ----------------------------------------------------------------------------------------------
    
    # -- Specify the iterations at which to compute per-parameter acceptance rates and adjust step sizes
    #check_steps=range(6000,11000,1000) # values of i at which to check the step sizing
    #check_steps=range(600, 1100, 100) # shorter intervals for testing
    
    # -- Build a data structure for storing the adaptive step size info
    #steps_log=np.zeros(shape=((len(check_steps)+1), len(steps)))
    
    # -- Put the first step sizes in row 0
    #steps_log[0]=steps 
    ...
    #it_names = ['iteration']
    #s_names = ['step_size_phi', 'step_size_rho'] # individual parameter step sizes may be different
    ...
    # ----------------------------------------------------------------------------------------------
    # -- PARTIALLY DEPRECATED Step size adjustment logs
    # ----------------------------------------------------------------------------------------------
    
    # -- Build a data structure for storing the adaptive step size info
    # -- storage vectors for column headers
    is_updated=[]
    Paccept_reject=[]
    #s_names=[]
    #a_names=[]
    
    # -- populate the column headers vectors with names
    #for idx in est_index:
        
        # -- 'is_updated' will actually go in the main chain, but we build the names vector here
    #    is_updated.append('is_updated_' + par_names[idx])
    #    Paccept_reject.append('par_accept_reject_' + par_names[idx])
        
        # -- the following names vector will go in the Adaptive Step sizes log
    #    s_names.append('par_step' + par_names[idx])
    #    a_names.append('accept_rate' + par_names[idx])
    
    # -- one last name for the step sizes data structure, and then initialize the array of the proper dimensions
    #it_names = ['iteration']
    #it_log = np.zeros(shape=(len(check_steps)+1, 1)) 
    #step_size_names=np.concatenate((it_names, s_names, a_names))
    
    # -- start a counter for how many times we've checked the step size
    #cs = 0
    
    # -- initialize the numpy arrays to store the adaptive step size info
    ...
    #steps_log=np.zeros(shape=((len(check_steps)+1), len(s_names)))
    #a_rates=np.zeros(shape=((len(check_steps)+1), len(a_names)))
    
    # -- put the first step sizes in row 0
    #steps_log[0] = steps #redundant here; this is done above
    ...
    proposed_log_names=np.concatenate((prop_par_names, prop_model_names, prop_prev_names, prop_lik_name, prop_prior_n, prop_posterior, update_index, par_bounds, prop_lik_in_bounds, prop_t_eq))
    ...
    fail_count=0
    for i in range(50000): # generate a pilot proposal distribution
...
# start the MCMC
MCMC(prev_data, dir_name, 20000, iterations, pop_sizes, racc_pop, plot=False)
